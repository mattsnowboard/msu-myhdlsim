...
class TestXorGate(TestCase):
            ...
            for i in range(2):
                ...
                for j in range(2):
                    ...
                    for k in range(2):
                        ...
                        yield delay(10)
                        #print a, b, c, out
                        self.assertEqual(not(a ^ b ^ c), out)
                ...
                for j in range(2):
                    ...
                    for k in range(2):
                        ...
                        for l in range(2):
                            ...
                            yield delay(10)
                            #print a, b, c, d, out
                            self.assertEqual(not(a ^ b ^ c ^ d), out)

        a, b, c, d, out = [Signal(None) for i in range(5)]
        gate = Nxor(out, a, b, c, d)
        check = test(out, a, b, c, d)
        sim = Simulation(gate, check)
        sim.run(quiet=1)
        
    def testUndefinedOneOfTwo(self):
        """ Make sure NXOR gate properly outputs None when given None """
        def test(out, a, b):
            yield delay(10)
            for i in range(2):
                a.next = i
    ...
    def testUndefinedOneOfFour(self):
        """ Make sure NXOR gate properly outputs None when given None for 4 bits """
        def test(out, a, b, c, d):
            yield delay(10)
            for i in range(2):
                a.next = i

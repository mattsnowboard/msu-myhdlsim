...
from MyHDLSim.combinational import Xor
...
class TestXorGate(TestCase):
    ...
    def testFourBits(self):
        ...
        gate = Xor(out, a, b, c, d)
        check = test(out, a, b, c, d)
        sim = Simulation(gate, check)
        sim.run(quiet=1)
        
    def testUndefinedOneOfTwo(self):
        """ Make sure XOR gate properly outputs None when given None """
        def test(out, a, b):
            yield delay(10)
            for i in range(2):
                a.next = i
    ...
    def testUndefinedOneOfFour(self):
        """ Make sure XOR gate properly outputs None when given None for 4 bits """
        def test(out, a, b, c, d):
            yield delay(10)
            for i in range(2):
                a.next = i
